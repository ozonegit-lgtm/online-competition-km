...
<div class="grid gap-4 lg:grid-cols-2">
    <div class="lg:col-span-2">
        ...
        <input id="title" name="title" required maxlength="255" value="{{ old('title', $knowledgeItem->title ?? '') }}" class="mt-2 w-full rounded-xl border-slate-300 text-sm border py-2 h-10 px-3">
        @error('title') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
    </div>
    <div class="lg:col-span-2">
        <label for="category_id" class="block text-sm font-semibold text-slate-700">หมวดหมู่</label>
        <select id="category_id" name="category_id" required class="mt-2 w-full rounded-xl border-slate-300 text-sm border py-2 h-10 px-3">
    ...
    <div class="lg:col-span-2">
        ...
        <textarea id="summary" name="summary" rows="4" class="mt-2 w-full rounded-xl border-slate-300 text-sm border px-3 py-2">{{ old('summary', $knowledgeItem->summary ?? '') }}</textarea>
    ...
    <div class="lg:col-span-2">
        ...
        <textarea id="content" name="content" rows="10" class="mt-2 w-full rounded-xl border-slate-300 text-sm border px-3 py-2">{{ old('content', $knowledgeItem->content ?? '') }}</textarea>
        @error('content') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
    </div>
    <div>
        <label for="cover_image" class="block text-sm font-semibold text-slate-700">รูปปก</label>
        <input id="cover_image" name="cover_image" type="file" accept="image/jpeg,image/png,image/webp" class="mt-2 block w-full rounded-xl border border-slate-300 p-2 text-sm px-3 h-10">
        <p class="mt-1 text-slate-500 text-xs">JPG, JPEG, PNG หรือ WEBP ไม่เกิน 10 MB</p>
    ...
    <div>
        ...
        <input id="attachment" name="attachment" type="file" accept=".jpg,.jpeg,.png,.webp,.pdf,.doc,.docx,.ppt,.pptx,.zip" class="mt-2 block w-full rounded-xl border border-slate-300 p-2 text-sm px-3 h-10">
        <p class="mt-1 text-slate-500 text-xs">รูปภาพ, PDF, Word, PowerPoint หรือ ZIP ไม่เกิน 10 MB</p>
