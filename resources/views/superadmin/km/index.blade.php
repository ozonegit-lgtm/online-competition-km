            ...
            <article class="group flex h-full flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition hover:border-slate-300 hover:shadow-md">
                ...
                <div class="flex flex-1 flex-col p-3">
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <span class="rounded-full px-2 py-0.5 text-[10px] font-semibold {{ $statusConfig['class'] }}">{{ $statusConfig['label'] }}</span>
                    ...
                    <h3 class="mt-2 line-clamp-2 text-sm font-semibold leading-5 text-slate-900 transition group-hover:text-blue-700">{{ $item->title }}</h3>

                    @if($item->summary)
                        <p class="mt-1 line-clamp-2 text-xs leading-4 text-slate-500">{{ $item->summary }}</p>
                    @endif
        ...
        @foreach($submissions as $submission)
            ...
            <article id="km-submission-{{ $submission->id }}" class="group flex h-full flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition hover:border-slate-300 hover:shadow-md">
                ...
                <div class="flex flex-1 flex-col p-3">
                    ...
                    <h3 class="mt-2 line-clamp-2 text-sm font-semibold leading-5 text-slate-900 transition group-hover:text-violet-700">{{ $submission->project_title }}</h3>
                    ...
                    @if($submission->project_description)
                        <p class="mt-1 line-clamp-2 text-xs leading-4 text-slate-500">{{ $submission->project_description }}</p>
                    @endif
