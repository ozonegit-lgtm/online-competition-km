...
<article class="mx-auto max-w-5xl overflow-hidden rounded-2xl border bg-white shadow-sm">
    @if($knowledgeItem->cover_image)<img src="{{ Storage::disk('public')->url($knowledgeItem->cover_image) }}" alt="{{ $knowledgeItem->title }}" class="h-64 w-full object-cover">@else<div class="flex h-40 items-center justify-center bg-slate-100 text-slate-400">ไม่มีรูปปก</div>@endif
    <div class="space-y-6 p-5 sm:p-8">
        ...
        @if($knowledgeItem->attachment_path)<a href="{{ Storage::disk('public')->url($knowledgeItem->attachment_path) }}" class="inline-flex rounded-xl border px-4 py-2 text-sm font-semibold">{{ $knowledgeItem->attachment_original_name ?: 'ดาวน์โหลดไฟล์แนบ' }}</a>@endif
