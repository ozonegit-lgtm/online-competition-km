    ...
    <div class="flex items-center gap-3">
        <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-blue-600 ring-1 ring-blue-100">
            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-16ZM8 7h8M8 11h6"/>
            </svg>
        </div>

        <div>
            <h1 class="text-xl font-bold tracking-tight text-slate-900">จัดการองค์ความรู้</h1>
...
<div id="km-list" class="space-y-4">
    ...
    <form
        method="GET"
        action="{{ route('competition-admin.km.index') }}"
            ...
            <div class="flex items-start gap-3">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-slate-500">
                        ค้นหาจากชื่อหรือรหัสผลงาน แล้วจำกัดผลลัพธ์ด้วยหมวดหมู่ สถานะ และประเภทข้อมูล
                    </p>
                </div>
            </div>
                ...
                <div class="xl:col-span-6">
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            placeholder="ชื่อผลงาน รหัสผลงาน หรือการแข่งขัน"
                        ...
                        <div class="flex flex-wrap items-center gap-1.5">
                            ...
                            @if(request('search'))
                                <span class="inline-flex max-w-full items-center rounded-md bg-slate-100 px-2 py-1 text-[11px] font-medium text-slate-600">
                                    คำค้นหา:
                                    <span class="ml-1 max-w-[220px] truncate font-semibold text-slate-800">
                                        {{ request('search') }}
                                    </span>
                                </span>
                            @endif
                            ...
                            @if(request('source'))
                                <span class="inline-flex items-center rounded-md bg-slate-100 px-2 py-1 text-[11px] font-medium text-slate-600">
                                    ประเภท:
                                    <span class="ml-1 font-semibold text-slate-800">
                                        {{ request('source') === 'competition' ? 'ผลงานการแข่งขัน' : 'องค์ความรู้' }}
                                    </span>
                                </span>
                            @endif
                        </div>
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-xs text-slate-500">
                องค์ความรู้และผลงานจากการแข่งขันที่อยู่ในความดูแลของคุณ ·
                <span id="km-total">{{ number_format($visibleCount) }} รายการในหน้านี้</span>
            </p>
        </div>
                    ...
                    <div class="mt-auto grid grid-cols-2 gap-1.5 border-t border-slate-100 pt-2 sm:grid-cols-3">
                        @can('view', $item)
                            <a
                                ...
                                class="inline-flex h-8 w-full items-center justify-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-700 transition hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-300"
                            >
                                <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                        ...
                        @can('update', $item)
                            <a
                                href="{{ route('competition-admin.km.edit', $item) }}"
                                class="inline-flex h-8 w-full items-center justify-center gap-1.5 rounded-lg border border-blue-200 bg-blue-50 px-3 text-xs font-semibold text-blue-700 transition hover:bg-blue-100 focus:outline-none focus:ring-2 focus:ring-blue-300"
                            >
                                <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                                    <path d="M12 20h9"/>
                        ...
                        @if($item->status === 'published')
                            @can('unpublish', $item)
                                ...
                                >
                                    <button
                                        type="submit"
                                        class="inline-flex h-8 w-full items-center justify-center gap-1.5 rounded-lg border border-amber-200 bg-amber-50 px-3 text-xs font-semibold text-amber-700 transition hover:bg-amber-100 focus:outline-none focus:ring-2 focus:ring-amber-300"
                                    >
                                        <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                                            <path d="M3 3l18 18"/>
                        ...
                        @else
                            @can('publish', $item)
                                ...
                                >
                                    <button
                                        type="submit"
                                        class="inline-flex h-8 w-full items-center justify-center gap-1.5 rounded-lg bg-emerald-600 px-3 text-xs font-semibold text-white transition hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-400"
                                    >
                                        <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                                            <path d="M3 12s3.5-6 9-6 9 6 9 6-3.5 6-9 6-9-6-9-6Z"/>
                                            <circle cx="12" cy="12" r="2.5"/>
                        ...
                        @can('delete', $item)
                            <x-ajax-form
                                target="#km-list, #km-total"
                                :action="route('competition-admin.km.destroy', $item)"
                                method="DELETE"
                            ...
                            >
                                <button
                                    type="submit"
                                    class="inline-flex h-8 w-full items-center justify-center gap-1.5 rounded-lg border border-red-200 bg-white px-3 text-xs font-semibold text-red-600 transition hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-300"
                                >
                                    <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                                        <path d="M3 6h18"/>
                                        <path d="M8 6V4h8v2"/>
                ...
                <div class="flex flex-1 flex-col p-3">
                    ...
                    <div class="mt-2 grid grid-cols-2 gap-2 rounded-xl bg-slate-50 px-2.5 py-2">
                        <div class="min-w-0">
                            ...
                            <p class="mt-1 truncate text-xs font-medium text-slate-700">
                                {{ $submission->competition?->title ?? 'ไม่พบการแข่งขัน' }}
                            </p>
                        </div>
                    ...
                    <div class="mt-auto grid grid-cols-2 gap-1.5 border-t border-slate-100 pt-2 sm:grid-cols-3">
                        @if($knowledgeItem)
                            @can('view', $knowledgeItem)
                                <a
                                    href="{{ route('competition-admin.km.show', $knowledgeItem) }}"
                                    class="inline-flex h-8 w-full items-center justify-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-700 transition hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-300"
                                >
                                    <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                                        <path d="M3 12s3.5-6 9-6 9 6 9 6-3.5 6-9 6-9-6-9-6Z"/>
                        ...
                        @if($isPublished)
                            <x-ajax-form
                                ...
                                success="ถอนผลงานออกจาก KM เรียบร้อยแล้ว"
                                class="w-full"
                            >
                                <button
                                    type="submit"
                        ...
                        @else
                            <x-ajax-form
                                ...
                                success="เผยแพร่ผลงานสู่ KM เรียบร้อยแล้ว"
                                class="w-full"
                            >
                                <button
                                    type="submit"
        ...
        @if(!$hasKnowledgeItems && !$hasSubmissions)
            <div class="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white px-4 py-8 text-center shadow-sm md:col-span-2 xl:col-span-3">
                ...
                <p class="mt-1 text-xs text-slate-400">
                    ไม่พบองค์ความรู้หรือผลงานการแข่งขันที่ตรงกับเงื่อนไข
                </p>

                @if($activeFilterCount > 0)
                    <a
                        href="{{ route('competition-admin.km.index') }}"
                        class="mt-4 inline-flex h-8 items-center justify-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-600 transition hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-300"
                    >
                        ล้างตัวกรอง
                    </a>
                @endif
